 ...
 */
use report_trainingenrolment\form\date_filter;
...
require_capability('report/trainingenrolment:view', $context);

$heading = get_string('trainingenrolment', 'report_trainingenrolment');
$url = new moodle_url('/report/trainingenrolment/index.php');
...
$PAGE->navbar->add(get_string('training_report', 'report_trainingenrolment'), $url);
$PAGE->navbar->add(get_string('view_report', 'report_trainingenrolment'));
echo $OUTPUT->header();

$admin_enrol_values = $admin_incomplete_values = $admin_incomplete_allvalues = [];
$currentdate = date('d/m/Y');
$today = strtotime(date('d-m-Y'));

//Captures formdata
$datefilter_form = new date_filter();
$datefilter_form->display();
if ($formdata = $datefilter_form->get_data()) {
    $selected_date = userdate($formdata->dateselect, '%d/%m/%Y');
    $selected_raw = $formdata->dateselect;
    $yesterday = $formdata->dateselect - 86400;
} else {
...
echo $OUTPUT->render_from_template('report_trainingenrolment/tabledata', $templatecontext);
$PAGE->requires->js_call_amd('report_trainingenrolment/tabledata', 'init');

//Adds report view event into logs
report_trainingenrolment_events::report_viewed($USER->id);
echo $OUTPUT->footer();
